...
export const todoService = {
  ...
  create: async(input: any, userId: string)=> {
    ...
    return Todo.create(buildTodoInput(input, userId));
  },
   ...
   update: async(input: any, userId: string) =>{
    ...
    return todo.update(buildTodoInput(input, userId));
  },
  ...
  addDescription: async(description: string, userId: string, todoId: string)=> {
  ...
  createSubTask: async(parentId: string, input: any, userId: string) =>{
   ...
   getSubTasks:async(todoId: string, userId: string)=> {
    return Todo.findAll({ where: { parentId: todoId, userId } });
  },

   getProjectTodos:async(projectId: string, userId: string)=> {
    return Todo.findAll({ where: { projectId, userId } });
  },
  ...
  addSevenDayTodo: async(input: any, userId: string)=> {
   ...
   createRecurringTodos: async(input: any, userId: string)=> {
    ...
    const todos: any[] = [];
    const initialDate = input.startDate ? new Date(input.startDate) : new Date();
    let date = new Date(initialDate);

    // Example limit: generate next 12 occurrences (you can change or make it param)
  ...
  setReminder: async(todoId: string, userId: string, date: Date)=> {
    ...
    return todo.update({ reminderDate: date });
  },
  ...
  async filterTodos(query: any, userId: string) {
    ...
    if (query.startDateFrom || query.startDateTo) {
      where.startDate = {};
      if (query.startDateFrom) where.startDate[Op.gte] = new Date(query.startDateFrom);
      if (query.startDateTo) where.startDate[Op.lte] = new Date(query.startDateTo);
    }

    if (query.dueDateFrom || query.dueDateTo) {
      where.dueDate = {};
      if (query.dueDateFrom) where.dueDate[Op.gte] = new Date(query.dueDateFrom);
